...
class MotifPromptedGINEncoder(GINs.GINEncoder):
    ...
    def forward(self, x, edge_index, batch, batch_size, **kwargs):
        ...
        for i, (conv, batch_norm, relu, dropout) in enumerate(
            ...
            post_conv = self.prompts[i](post_conv, batch)
...
def train_batch(data, config, optimizer, model) -> dict:
    ...
    prompts = nn.ModuleList(
        [
            ...
            for _ in range(config.model.model_layer - 1)
        ]
    )
...
def main(config_name, config_path, seed: int):
    MotifPromptedGINEncoder.enable()
    # load config for yml
    args = args_parser(
        [
